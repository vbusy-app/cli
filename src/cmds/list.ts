...
import { type Task, formatDueDate } from "../utils/index.js";
...
export default class ListCmd extends Command {
	...
	public async run(argv: Argv): Promise<void> {
		...
		const tasks = (await this.vbusy.api.taskRepository.getAll(
			token as string,
		)) as Task[];
		...
		const table = new Table({
			head: [
				chalk.bgYellow.black("ID"),
				chalk.bgYellow.black("Completed"),
				chalk.bgYellow.black("Due"),
				chalk.bgYellow.black("Priority"),
				chalk.bgYellow.black("Task"),
			],
		...
		for (const task of tasks) {
			...
			const taskName = task.archived
				? chalk.gray(`[archived] ${task.task}`)
				: task.task;
